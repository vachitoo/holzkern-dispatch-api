...
app.get('/api/dispatch', async (req, res) => {
  try {
    ...
    const warehouse = {};
    for (const key of ['v03','v04','v05','v06','v09']) {
      const ids = (ptypes||[]).filter(p=>((p.warehouse_id?.[1]||'')+(p.name||'')).toLowerCase().includes(key)).map(p=>p.id);
      warehouse[key] = ids.length ? await cnt('stock.picking',[['picking_type_id','in',ids],['state','not in',['done','cancel']]]) : null;
    }

    res.json({
      dispatch: {
        open: openAll, done_today: doneToday, express, prio,
